...
show_dotfiles() {
    ...
    print_cmd "dotfiles review" "Review, commit, and push pending changes"
    printf "    %-20s ${DIM}%s${NC}\n" "" "Run when terminal nudges you or after daemon adopts new configs"
    ...
    echo ""
    echo -e "  ${DIM}Auto-adopt daemon${NC}"
    print_cmd "dotfiles status" "Check daemon status"
    print_cmd "dotfiles log [n]" "Show last n log entries"
    print_cmd "dotfiles run-now" "Manually trigger adoption scan"
